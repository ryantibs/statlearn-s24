...
$R_{n+1}$ being $\leq$ the \smash{$\lceil (1-\alpha)(n+1) \rceil$} 
...
\smash{$\hC^{w,*}_n$} defined above (they are actually equivalent) satisfies        
\[
...
\citet{barber2023conformal}. In comparison to the likelihood-weighted method
...
\smash{$\hC^{w,*}_n$} satisfies the same guarantee as in
...
Let $w_i \in [0,1]$, $i=1,\dots,n$ be fixed and arbitrary weights, and define
\smash{$\tw_i$}, $i=1,\dots,n+1$ as in \eqref{eq:tw}. Define a weighted
conformal set (based on the first $n$ samples) at a point $x \in \cX$, with
nominal error level $\alpha \in (0,1)$, by    
\begin{equation}
...
\bigg) \bigg\},   
\end{equation}
where \smash{$R_i^{(x,y),K}$}, $i=1,\ldots,n+1$ are conformity scores defined as
follows. Let \smash{$Z^{(x,y)} = (Z_1,\dots,Z_n,(x,y))$}, and let
\smash{$(Z^{(x,y)})^K$} denote this vector after swapping components $K$ and  
$n+1$, where
\[
...
distribution of $Z_i=(X_i,Y_i)$, $i=1,\dots,n+1$, the set \smash{$\hC^w_n$}
...
follows a multinomial distribution, with probabilities \smash{$\tw_i$},
...
\smash{$C_t^\beta \subseteq \cY$} for $Y_t$, at any nominal level $\beta \in
\R$. The set \smash{$C_t^\beta$} could come from some version of conformal 
...
Adaptive conformal inference (ACI)\footnote{The method was proposed and named by
  ...
  method used to compute prediction sets \smash{$C_t^\beta$}, but as noted, any
...
\end{equation}
where \smash{$\err_t = 1\{ Y_t \notin C_t^{1-\alpha_t} \}$} and $\eta>0$ is a
...
coverage guarantee over any interval $[t_0+1, t_0+T]$ of \smash{$\approx
  1-\alpha \pm \frac{1}{T}$}, as seen in \eqref{eq:aci_cov_finite}. This of
