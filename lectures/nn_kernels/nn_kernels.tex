...
% random). In general, in nonparametric regression, we study error\footnote{In
% the review lecture, we were more precise about using ``error'' and ``risk'' to 
%   mean separate things; and defined the risk via the expectations
%   \smash{$\E\|\hf - f_0\|_n^2$} and \smash{$\E\|\hf - f_0\|_2^2$}, with
%   respect to appropriate randomness in the training set that is used to define 
%   \smash{$\hf$}. In this and future lectures, we'll often take the liberty of
%   being more flexible with our nomenclature.}    
% bounds in probability or in expectation, depending on what is more convenient
% in the given analysis. Upper bounds on the risk (as we will see in this
% lecture and the next) are usually easier to derive in probability, and lower
% bounds (as we will see in the minimax theory lecture) are typically easier to
% derive in expectation. We tend not to fuss about this discrepancy,
% particularly because upper bounds in probability can be translated into upper
% bounds in expectation whenever the probability control is sharp enough (as you
% saw on the homework).   
...
% sufficiently flexible---otherwise we can't guarantee \smash{$\hf$} will be
% close $f_0$. The gap between our representation and $f_0$ is known as the 
% \emph{approximation error}. For traditional nonparametric theory (e.g., theory 
% for $k$-nearest neighbors and kernel regression), this won't be an explicit
% part of the analysis; but for some advanced results, it will (we'll touch on
% this in a later lecture). Here is a concrete example of an approximation
% guarantee: for any $f_0 : [0,1] \to \R$ whose second derivative is integrable,
% and evenly-spaced points $t_1,\dots,t_N \in [0,1]$, there is a cubic spline 
% \smash{$\bar{f}$} with knots at $t_1,\dots,t_N$ (to be defined precisely in
% the splines lecture) such that     
% \[
...
% is a measure of the smoothness of $f_0$. If this remains constant, and we
% choose \smash{$N=\sqrt{n}$}, then the squared $L^2$ approximation
% error---either in empirical or population norm (\smash{$\|\bar{f} -
% f_0\|_n^2$} or \smash{$\|\bar{f} - f_0\|_2^2$})---will be on the order of
% $1/n$, which will be negligible relative to the overall estimation error that
% we will encounter. 
...
inputs $x_1,\dots,x_n$, but not on $y_1,\dots,y_n$. The class of linear
...
\eqref{eq:kernel} is a smooth moving average. See Figure \ref{fig:kernel} for an example.
...
% RJT. Small discrepancy, because [0,1]^d is not open and it's easiest to define
% a Holder space over an open domain (as otherwise what do you mean by being
% differentiable at the boundary?). But .... for consistency with the Lipschitz
% statement and not to propogate a lot of changes now we can just leave it as
% is. And, even if we wanted to precise it's fine, as we could just define the
% Lipschitz condition (in the Holder space definition) with respect to the
...
\[ 
\int K(t) \, dt = 1, \quad
\int t^j K(t) \, dt = 0, \; j=1,\dots,k-1, \quad
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.5\textwidth]{order4.pdf}
