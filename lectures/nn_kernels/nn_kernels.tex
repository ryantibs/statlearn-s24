...
the distribution of $x_{1:n}$, we will have \smash{$\|x_i   - x_0\|_2 \leq
  c(k/n)^{1/d}$} for all $i \in \cN_k(x_0)$, where $c>0$ is a constant not
depending on $x_0$. (You will prove this on the homework.) Then our
...
set $S$ that is not ``too small'', where $c>0$ is a constant. Our bias-variance 
