...
estimator (in a specific case). But when $2m < d$, problem
...
important? Well, given a sequence such that $f_k \to f$ as $k \to \infty$ in
\smash{$\|\cdot\|_{W^{m,2}(U)}$} norm, the continuous embedding property implies
$f_k \to f$ as $k \to \infty$ in $L^\infty$ norm, which implies $f_k(x) \to
...
But when $2m < d$, we do not get an embedding into $C^0(U)$, and it turns out 
that: 
\begin{quote}
\centering\it
For $2m < d$, the point evaluation operator is not continuous on $W^{m,2}(U)$.     
\end{quote}
This means that there can be a sequence such that $f_k \to f$ as $k \to \infty$
in \smash{$\|\cdot\|_{W^{m,2}(U)}$} norm but $f_k(x) \not\to f(x)$ for some
$x$. In fact, it's not hard to see this from first principles. Let $f$ be any
infinitely differentiable ``bump'' function which is unimodal with a unique max 
at the origin, of $f(0) = 1$, and which is zero outside of the unit $\ell_2$
ball $\{x : \|x\|_2 \leq 1\} \subseteq U$. Define $f_k(x) = f(kx)$, for
...
\smash{$\|\cdot\|_{W^{m,2}(U)}$} norm. However, recall $f_k(0) = 1$ for all $k$,
so clearly we do not get pointwise convergence at the origin. 

The lack of well-defined point evaluations means that we \emph{cannot} generally
do things like solve the variational problem \eqref{eq:ss_multivariate}.

% RJT. What happens at 2m = d? I believe point evaluation continues to be
% ill-defined, in the sense that the point evaluation operator is not
% continuous. It requires a different construction, with a log singularity. See
% e.g., the proof of the second case on page 8 of El Alaoui et al. (2016):   
% https://proceedings.mlr.press/v49/elalaoui16.pdf. 
% In our notation, this sets k = 1 and p = d, so it would need to be adapted,
% and I should see the details through. Also, some of Dejan's papers carry this
% idea forward. Thanks to Alden for reminding me of all this.  

% Another interesting note, Chapter 17 of Tartar (``An Introduction to Sobolev 
% Spaces'') proves the result for m = 1 and d = 2 in a couple of different ways,
% this is another candidate for extending it to higher dimensions.
 
% Meanwhile, here is an interesting twist: for p = k = d = 1, the Sobolev space 
% W^{1,1}(\R) admits a precise representative for each function that is
% absolutely continuous on compact subintervals of \R. Such a representative is
% continuous, and hence point evaluation returns to being well-defined. 

% In other words, at the critical boundary, pk = d, it seems the conclusion is
% that point evaluation is not well-defined in general but can be defined in
% particular cases. 
...
% RJT. Again I don't love this because when U = \R^d, we're not really doing
% minimization over the Sobolev space below. So this is kind of murky territory.  
...
Sobolev space is continuous if $2m > d$. Therefore, we learn the following:        
\emph{a Sobolev space $W^{m,2}(U)$ is an RKHS if $2m > d$}. This is an amazing
fact. 
...
% RJT. This is kind of cheating, but by putting it in quotes, I feel OK as I'm
% being sufficiently vague about what the actual equivalence is.   
