...
# Sig = matrix(rho, p, p); diag(Sig) = 1 # equicorrelated
...
r = min(n, p)
e.hat = eigen(Sig.hat)
Sig.hat.pinv = e.hat$vec %*% diag(c(1 / e.hat$val[1:r], rep(0, p-r))) %*% t(e.hat$vec)
N = (Id - Sig.hat.pinv %*% Sig.hat)
b0.emp = t(b0) %*% N %*% Sig %*% N %*% b0 * (n < p)
v0.emp = sum(diag(Sig.hat.pinv %*% Sig)) / n
  
# Asymptotic bias and variance: ridge
gam = p/n
s = e$val

# Function for solving the Silverstein equation
...
# Asymptotic bias and variance: ridgeless
...
matplot(lam.vec, cbind(r.emp, r.lim), type = "l", lty = 1:2, col = 1:2, 
...
legend("topleft", c("Finite-sample", "Asymptotic"), lty = 1:2, col = 1:2)
graphics.off()
