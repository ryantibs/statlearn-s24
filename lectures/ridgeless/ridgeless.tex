...
\smash{$\Risk_X(\hbeta; \beta_0, \theta_0)$} in \eqref{eq:risk_mis}, and
...
misspecified setting decomposes into two terms: \smash{$\tilde{R}_X(\hbeta;
  \beta_0, \theta_0)$}, measuring how well we can predict the conditional mean
...
% (note that it does not depend at all on $X$ or \smash{$\hbeta$}).    
...
calculations. The challenge is that \smash{$\lambda^2 (\hSigma + \lambda I)^{-1}
  \Sigma (\hSigma + \lambda I)^{-1}$} (ridge) and \smash{$(I - \hSigma^+
  \hSigma) \Sigma (I - \hSigma^+ \hSigma)$} (ridgeless) are complex functions of
...
functional \smash{$\lambda^2 (\hSigma + \lambda I)^{-1} \Sigma (\hSigma +
...
can be explained as follows: the empirical bias \smash{$B_{\hSigma}$} at
