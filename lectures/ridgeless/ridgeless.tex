...
\eqref{eq:ridgeless} as \smash{$\hbeta = (X^\T X/n)^+ X^\T Y/n$}, and record the
...
To derive the asymptotic risk for the ridgeless estimator \smash{$\hbeta_0 =
  (X^\T X)^+ X^\T Y$}, we can just take $\lambda \to 0$ in
...
For the bias, recall that for the ridge estimator \smash{$\hbeta_\lambda = (X^\T
  X + n \lambda I)^{-1} X^\T Y$} for $\lambda>0$, we have 
\begin{equation}
\label{eq:ridge_bias}
...
that the functional \smash{$\lambda^2 (\hSigma + \lambda I)^{-1} \Sigma (\hSigma
  + \lambda I)^{-1}$} does not has a transparent deterministic equivalent. There
...
This yields for the ridgeless estimator \smash{$\hbeta_0 = (X^\T X)^+ X^\T
  Y$},   
\begin{equation}
...
calculations, this completes the picture for the asymptotic risk of ridge and
ridgeless regression. For concreteness, the results (writing the bias terms in 
...
\emph{ridge risk curve will be very flat for small $\lambda$, and achieve its
  minimum at $\lambda^* = 0$}. Figure \ref{fig:ridge_ar} gives an example with
the autoregressive feature model.

Beyond empirical evaluation of the deterministic risk equivalents, formal
arguments can be made about the shape of the asymptotic risk, and location of
its minimizer $\lambda^*$, as done in \citet{wu2020optimal,
  richards2021asymptotics} under certain alignment conditions on $\Sigma$ and
$\beta_0$. Forthcoming work will show that this type of analysis be carried out
in greater generality (even under distribution shift). 
...
the observed features are noisy pertubations of these latent features. This is a
simple ``toy'' model for what actually happens in methods which learn feature 
representations, like neural networks, which in truth are much more complex. But
may be a useful model to keep in mind nonetheless, as we continue to chip away
in understanding interpolation in modern prediction. 

\begin{figure}[tb]
\centering
\includegraphics[width=0.85\textwidth]{ridge_ar.pdf}
\caption{\it The finite-sample risk of ridge and ridgeless regression (black) 
  plotted alongside the deterministic equivalents (red) in
  ...
  eigenvector of $\Sigma$. We set the signal-to-noise ratio to be $\mathrm{SNR}  
  = \|\beta_0\|_2^2 / \sigma^2 = 3.33$, and set $n = 80$ and $p = 500$. In this  
  example, both the finite-sample and asymptotic risk curve achieve their minima
  at $\lambda^* = 0$.}         
\label{fig:ridge_ar}
\end{figure}

% RJT. We should put in the df versions here as a last paragraph. Credit Francis
% Bach but also Sollich. Ask Pratik for help assembling what to write here.  
