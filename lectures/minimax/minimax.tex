...
Indeed, we can make this idea explicit. Fix any small $\epsilon>0$, let $f,g \in
W^{s,2}( (1-\epsilon) L; [0,1]^d)$ be any two functions with \smash{$\|f -
  g\|_{L^2([0,1]^d)} > 0$}. Suppose Let $\delta_i = g(x_i) - f(x_i)$, for $i =
1,\dots,n$, and let us perturb the evaluations of $g$ by defining    
\[
h_{i,N} = \delta_i \cdot h(N(x-x_i)), \quad i=1,\dots,n.
\]
...
the unit ball, with $h(0) = 1$. When $2m = d$, recall, we would have to modify
this construction somewhat (by taking an average of bump functions at different 
scales), but the same properties that will be used in what follows would still
hold. 

Then by the calculation given last time, each
\smash{$\|h_{i,N}\|_{W^{s,2}([0,1]^d)} \to 0$} as $N \to \infty$. So defining  
\[
\tilde{g}_N = g + \sum_{i=1}^n h_{i,N},
\]
we find that for large \smash{$\|\tilde{g}_N - g\|_{W^{s,2}([0,1]^d)} \to 0$} as
$N \to \infty$. In other words, we have constructed a sequence of functions
whose evaluations becomes abitrarily close to those of $f$: 
\[
\lim_{N \to \infty} \, \max_{i=1,\dots,n} \, |f(x_i) - \tilde{g}_N(x_i)| = 0. 
\]
and yet remains bounded away from $f$ in $L^2$ norm (since convergence in
Sobolev norm implies converges in $L^2$ norm, of \smash{$\tilde{g}_N$} to $g$): 
\[
\lim_{N \to \infty} \, \| f - \tilde{g}_N \|_{L^2([0,1]^d)} = \|f -
g\|_{L^2([0,1]^d)} > 0. 
\]
It should therefore be clear that when $2s \leq d$ one cannot hope to estimate a
Sobolev function in $L^2$ norm by observing its evaluations at fixed points.       
