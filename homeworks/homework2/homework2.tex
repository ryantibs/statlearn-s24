...
\section{Properties of RKHS regression [32 points]}
...
\section{Minimax lower bounds for H{\"o}lder classes [30 points]}

Consider the $d$-dimensional H{\"o}lder ball    
\[
C^s(L; [0,1]^d) = \Big\{ f : [0,1]^d \to \R \,:\, | D^\alpha f (x) - D^\alpha
f(z) | \leq L \|x-z\|_2 \; \text{for all $|\alpha| = {s-1}$, and $x,z \in
  [0,1]^d$} \Big\},  
\]
where $s \geq 1$ is an integer and $L>0$ is a constant (not growing with $n$).
Consider i.i.d.\ samples $(x_i, y_i)$, $i=1,\dots,n$ where each
\[
y_i = f(x_i) + \epsilon_i, \quad x_i \sim \mathrm{Unif}([0,1]^d) \quad
\epsilon_i \sim N(0,\sigma^2), \quad \text{and} \quad x_i \indep \epsilon_i.
\]
Generalize the calculations given in the minimax theory lecture for the minimax
lower bound over the Lipschitz ball to show that 
\marginpar{\small [30 pts]}
\[
\inf_{\hf} \, \sup_{f \in C^1(L; [0,1]^d)} \, \E \bigg[ \int_{[0,1]^d} (\hf(x) -
f(x))^2 \, dx \bigg] \gtrsim n^{-2s/(2s+d)}. 
\]
...
\[
g_\alpha(x) = Lh K \bigg( \frac{x - x_\alpha}{h} \bigg), 
\]
where $K$ is any smooth function supported on the unit $\ell_2$ ball $\{ x : 
\|x\|_2 \leq 1 \}$, with the following properties: $K(0) = 1$, \smash{$0 < \int
  K(x)^2 \, dx < \infty$}, and $D^\alpha K$ is 1-Lipschitz for all $|\alpha| <
s$ (we can equivalently assume bounded derivatives: $\|D^\alpha f\|_2 \leq 1$
for all $\alpha \leq s$). Then follow the calculations in the lecture, where we
used Fano's method and the Varshamov-Gilbert lemma.

\section{Sub-Gaussian maximal inequalities [24 points]}
...
\section{Risk analysis for the constrained lasso [24 points]}
...
\item Prove that the lasso estimator, with $t=\|\beta_0\|_1$, has in-sample risk
  ...
  \epsilon$ is mean zero sub-Gaussian with variance proxy $\sigma^2 \|a\|_2^2$.  
