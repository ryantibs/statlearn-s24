...
\[
g_\alpha(x) = Lh^s K \bigg( \frac{x - x_\alpha}{h} \bigg), 
\]
...
\|x\|_2 \leq 1 \}$, with the following properties: $K(0) = 1$, \smash{$0 < \int
  K(x)^2 \, dx < \infty$}, and $D^\beta K$ is 1-Lipschitz for all $|\beta| < s$
(we can equivalently assume bounded derivatives: $\|D^\beta f\|_2 \leq 1$ for
all $|\beta| \leq s$). Then follow the calculations in the lecture, where we
